...
export interface GenerateNewTokensInterface {
    (refreshToken: string, models: ModelFactoryInterface): Promise<TokenStructureResponse | ObjectKeyValue>;
}
...
export const getTokenConfiguration: Function = (): TokenConfigurationInterface => {
    return {
        ...
        refreshTokenExpiration: process.env.REFRESH_TOKEN_EXPIRATION ? process.env.REFRESH_TOKEN_EXPIRATION : '7d'
    }
}
...
export const generateNewTokens: GenerateNewTokensInterface = async (refreshToken: string, models: ModelFactoryInterface): Promise<TokenStructureResponse | ObjectKeyValue> => {
    try {
        ...
        const refreshSecret: string = process.env.REFRESH_TOKEN_SECRET ? process.env.REFRESH_TOKEN_SECRET : '';

        if (payload) {
            const user: UserInstance | null = await models.User.findOne({ where: payload.id, attributes: { include: ['password'] } });
            if (user && user.id) {
                let refreshKey: string = refreshSecret + user.password;
                try {
                    jwt.verify(refreshToken, refreshKey);
                    const newTokens: TokenStructureResponse | ObjectKeyValue = await generateTokens(user.id, models);
                    newTokens.user = user;
                    return newTokens;
                } catch (e) {
                    return {};
    ...
    return async (req: express.Request, res: express.Response, next: express.NextFunction): Promise<void> => {
        ...
        if (token) {
            try {
                ...
                if (payload) {
                    const user: UserInstance | null = await models.User.findOne({ where: { id: payload.id } });
                    if (user) {
                        req.user = user;
                    }
                }
                ...
                if (e.name === 'TokenExpiredError') {
                    const refreshTokenInstance: TokenInstance | null = await models.Token.findOne({ where: { refresh_token: refreshToken } });
                    if (refreshTokenInstance) {
                        if (!refreshTokenInstance.used) {
                            const newTokens: TokenStructureResponse | ObjectKeyValue = await generateNewTokens(refreshToken, models);
